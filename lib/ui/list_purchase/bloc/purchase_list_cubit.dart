...
class PurchaseListCubit extends Cubit<PurchaseListState> {
  PurchaseListCubit(this._purchaseRepository)
  ...
  void getPurchasesByMonth() async {
    ...
    final result = await _purchaseRepository.getPurchasesByMonth(
      state.selectedDate.month,
      state.selectedDate.year,
    );
    result.when(
      ok: (purchases) {
        List<Purchase> listPurchases = _filterPurchasesBySelectedDate(
          purchases,
        );
  ...
  List<Purchase> _filterPurchasesBySelectedDate(List<Purchase> purchases) {
    return purchases
        .where(
          (p) =>
              p.finalizedAt == null ||
              ((p.createdAt.month <= state.selectedDate.month &&
                      p.createdAt.year <= state.selectedDate.year) &&
                  (p.finalizedAt!.month >= state.selectedDate.month &&
                      p.finalizedAt!.year >= state.selectedDate.year)),
        )
        .toList();
  }
